---
title: "Yi Lui Data Analysis: Transfer Efficiency"
author: "MFK"
date: "25/08/2021"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## R Markdown

This is an R Markdown document. Markdown is a simple formatting syntax for authoring HTML, PDF, and MS Word documents. For more details on using R Markdown see <http://rmarkdown.rstudio.com>.

When you click the **Knit** button a document will be generated that includes both content as well as the output of any embedded R code chunks within the document. You can embed an R code chunk like this:

```{r setup}
pacman::p_load(dplyr,brms,tidyr,ggplot2)

```

## Kalanne's sequential transfer data

You can also embed plots, for example:

```{r,echo = FALSE}

df<-vroom::vroom("Downloads/ExperimentalData.txt",delim = "\t")
skimr::skim(df)
```



## Model

```{r}
model_fit<-brms::brm(data = df,log10CFU~1+NumberContacts+Gloves+ (1|Participant),family = lognormal(link = "identity", link_sigma = "log"))
summary(model_fit)

model_fit_g<-brms::brm(data = df,log10CFU~1+NumberContacts+Gloves+ (1|Participant))
summary(model_fit_g)
plot(model_fit, variable = c("NumberContacts"))
```



## Fluorescent powder
This imports Yi lui's data 

```{r Yi's data import}
dY<-vroom::vroom("data/Yi.data.longformat.20191121_censoring_handled.csv") %>% janitor::clean_names()

skimr::skim(dY)


# Individual powder area vs contact number and loading weight 
dY %>% 
  filter(surface=="Plastic" ) %>% 
  filter(id<20) %>% 
  ggplot()+
  geom_point(aes(x=contact_number,y=powder_area,colour=gloves,size=loading_weight))+
  facet_wrap(~id,scales="free")+
  hrbrthemes::theme_ipsum()
  
dY %>% 
  # filter(surface=="Plastic" ) %>% 
  # group_by(surface,contact_number,gloves) %>% 
  # summarise(Mean=mean(powder_area),SD=sd(powder_area))
  ggplot()+
  geom_violin(aes(x=as.factor(contact_number),y=raw_int_d,colour=gloves),draw_quantiles = c(0.25,0.5,0.95))+
  scale_y_continuous(trans = "log10")+
  facet_wrap(~surface,scales="free")

```

# Some investigative plots
```{r investigative plots}
#Plot log10(raw_int_d) vs contact_number for each ID and colour by gloves
dY%>%
filter(surface=="Plastic" ) %>%
ggplot()+
geom_point(aes(x=contact_number,y=raw_int_d,colour=gloves))+
scale_colour_brewer(palette="Set1")+
scale_y_continuous(trans="log10")+
facet_wrap(~id)+
xlab("Contact number")+
ylab("Intensity")+
theme_linedraw()->a

#save ggplot figure
# ggsave(a,"images/intensity_for_each_ID.png",device="png",units=c("cm"),width=30)
ggsave(a,filename = "images/intensity_for_each_ID_plastic.png",dpi=300, device="png",width = 60, height = 60, units = "cm")


```



# BRMS model fit
Here we fit a Bayesian Regression Model using BRMS to the data.

```{r BRM raw_int_d }
#Here we fit a BRM model from BRSM to the data, with raw_ind_d as the dependent variable
#We 
model_fit_g<-brms::brm(data = dY,raw_int_d~powder_area+contact_number+gloves+surface+(powder_area+contact_number+gloves+surface|id),
                       family = lognormal(link = "identity", link_sigma = "log"))
summary(model_fit_g)
plot(model_fit_g)

```














